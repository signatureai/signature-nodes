            ...
            try:
                ...
                if not workflow_data:
                    return web.json_response(
                        text="No workflow data provided", status=400
                    )
                ...
                error_msg = (
                    f"Error creating manifest: {str(e)}\n{traceback.format_exc()}"
                )
        ...
        async def submit_workflow(request: web.BaseRequest):
            try:
                ...
                if not jenkins_url or not jenkins_auth:
                    raise ValueError(
                        "JENKINS_URL and JENKINS_AUTH environment variables must be set"
                    )

                jenkins_url = (
                    jenkins_url.rstrip("/")
                    + "/job/Submit%20Workflow/buildWithParameters"
                )
                ...
                async for part in form_data:
                    if (
                        not isinstance(part, aiohttp.BodyPartReader)
                    ...
                    if part.filename:
                        ...
                        new_form_data.add_field(
                            field_name, content, filename=part.filename
                        )
                    else:
                            ...
                            try:
                                async with aiohttp.ClientSession() as session:
                                    async with session.get(
                                        ...
                                        logging.info(
                                            f"Cover image fetch status: {response.status}"
                                        )
                                        if response.status == 200:
                                            ...
                                            raise Exception(
                                                f"Failed to fetch image: {response.status}"
                                            )
                            except Exception as e:
                                logging.error(
                                    f"Error processing coverImageUrl: {str(e)}"
                                )
                                raise
                        else:
                ...
                async with aiohttp.ClientSession() as session:
                    ...
                    async with session.post(
                        jenkins_url, data=new_form_data, headers=headers
                    ) as resp:
                        if resp.status != 201:
                            logging.error(
                            ...
                            return web.json_response(
                                text="Workflow submission failed", status=502
                            )
                        return web.json_response(
                            text="Workflow submitted successfully", status=200
                        )
