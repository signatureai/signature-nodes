...
app.registerExtension({
  ...
  registerCustomNodes() {
    class SetNode extends LGraphNode {
      ...
      constructor(title) {
        ...
        this.addWidget(
          ...
          (s, t, u, v, x) => {
            ...
            if (this.widgets[0].value !== "") {
              this.title = "SIG Set_" + this.widgets[0].value;
            }
            this.update();
        ...
        this.onConnectionsChange = function (
          ...
          if (link_info && node.graph && slotType == 1 && isChangeConnect) {
            ...
            if (fromNode && fromNode.outputs && fromNode.outputs[link_info.origin_slot]) {
              ...
              if (this.title === "SIG Set") {
                this.title = "SIG Set_" + type;
              }
              if (this.widgets[0].value === "*") {
        ...
        this.findGetters = function (graph, checkForPreviousName) {
          ...
          return graph._nodes.filter(
            (otherNode) => otherNode.type === "SIG Get Node" && otherNode.widgets[0].value === name && name !== ""
          );
        };
      ...
      onRemoved() {
        const allGetters = this.graph._nodes.filter((otherNode) => otherNode.type == "SIG Get Node");
        allGetters.forEach((otherNode) => {
          if (otherNode.setComboValues) {
          ...
          },
          {
            ...
            callback: () => {
              const allGetters = this.graph._nodes.filter(
                (otherNode) => otherNode.type === "SIG Get Node" || otherNode.type === "SIG Set Node"
              );
              allGetters.forEach((otherNode) => {
      ...
      _drawVirtualLinks(lGraphCanvas, ctx) {
        ...
        if (!this.flags.collapsed) {
          var start_node_slotpos = [this.size[0], LiteGraph.NODE_TITLE_HEIGHT * 0.5];
        } else {
    ...
    class GetNode extends LGraphNode {
      ...
      constructor(title) {
        ...
        this.addWidget(
          ...
          {
            values: () => {
              const setterNodes = node.graph._nodes.filter((otherNode) => otherNode.type == "SIG Set Node");
        ...
        this.onRename = function () {
          ...
          if (setter) {
            ...
            this.title = "SIG Get_" + setter.widgets[0].value;
        ...
        this.findSetter = function (graph) {
          ...
          const foundNode = graph._nodes.find(
            (otherNode) => otherNode.type === "SIG Set Node" && otherNode.widgets[0].value === name && name !== ""
          );
          return foundNode;
        };
      ...
      getInputLink(slot) {
        ...
        if (setter) {
          ...
          const errorMessage = "No SIG Set Node found for " + this.widgets[0].value + "(" + this.type + ")";
